...
struct HomeView: View {
    ...
    var body: some View {
		ZStack {
				...
				ScrollView(showsIndicators: false) {
					...
					VStack {
						ForEach(0..<10) { Index in
							ZStack {
								CardView(PostCardTapped: $PostCardTapped, PostCardMoveUp: $PostCardMoveUp, BringUpDetail: $BringUpDetail, Namespace: Namespace, Index: Index)
							}
						}
					}
					.padding(.bottom, 70)
			...
			if BringUpDetail { //And if index = SelectedCardIndex
				DetailView(PostCardTapped: $PostCardTapped, PostCardMoveUp: $PostCardMoveUp, BringUpDetail: $BringUpDetail, Index: Index, Namespace: Namespace)
